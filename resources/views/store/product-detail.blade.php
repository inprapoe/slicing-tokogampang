                ...
                <div class="w-56 h-56 flex-shrink-0 rounded-lg md:mr-6 mx-auto mb-8 md:mb-0">
                    <div class="owl-carousel owl-theme">
                        <div class="relative bg-gray-100 w-56 h-56">
                            @if( !$product->is_in_stock || ($product->qty - @$deducter) <= 0 ) <a href="{{ route('store.products.show',['slug'=> $product->slug, alias()]) }}" class="absolute flex justify-center items-center w-full h-full bg-black bg-opacity-25 cursor-default z-10">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <div class="flex flex-wrap -m-1">
                        @if (!empty(@$productBookmark->landing_id))
                            <a href="{{ route('store.products.show.landing.page', ['slug' => $product->slug, 'alias' => alias()]) }}" target="_blank"><button class="px-4 py-1.5 self-end text-white rounded-md font-semibold text-sm" style="background-color: {{ $reseller->customStore->theme_color }};">Informasi Produk</button></a>
                        @endif
                        @if($product->supplier->facebook_messanger_scripts)
                            @php
                                $messanger_url = "https://m.me/" . substr($product->supplier->facebook_messanger_scripts, strpos($product->supplier->facebook_messanger_scripts, 'page_id') + 11,15);
                            @endphp
                            <a href="{{ $messanger_url }}" class="p-2 rounded-lg font-bold text-white text-center" style="background-color:{{ $reseller->customStore->theme_color }}" target="popup" onclick="window.open('{{ $messanger_url }}','popup','width=900,height=900'); return false;">Contact Product Owner</a>
                        @endif
                        @if (!empty($product->product_knowledge))
                            <a href="{{ route('supplier.products.knowledge.download', ['id' => $product->id, 'alias' => alias()]) }}"><button class="px-4 py-1.5 self-end text-white rounded-md font-semibold text-sm m-1" style="background-color: {{ $reseller->customStore->theme_color }};">Download Product Knowledge</button></a>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-4 xl:col-span-5">
            ...
            <form id="buy-now-form" action="" method="post" class="grid gap-6">
                ...
                <div class="grid gap-6 hidden" id="metode-pembayaran">
                    ...
                    <div class="">
                        <input class="border border-1 border-gray-400 text-md rounded-md p-1" style="text-transform: uppercase" name="voucher_code" type="text" placeholder="Kode Voucher / Nominal Potongan Harga">
                ...
                <div class="hidden">
                    ...
                    @if($product->fulfillment_type !== \App\Models\Product::INHOUSE)
                    <input type="hidden" id="warehouse" name="warehouse_id" value="" />
                    @endif
                </div>
            </form>
        </div>
...
<script>
    $('#buy-now-button').click(function(event) {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '{{ route("store.checkout.store",alias()) }}',
            data: formData,
            dataType: 'json',
            encode: true,
            beforeSend: function() {
            ...
            success: function(data) {
                if (data.success) {
                    var fbpData = {
                        content_category: '{{ optional($product->category)->name }}',
                        content_ids: ['{{ $product->name }}'],
                        currency: 'IDR',
                        num_items: parseInt($('input[name=qty]').val()),
                        value: parseInt($('input[name=price]').val())

                    };
                    fbq('track', 'InitiateCheckout', fbpData);
                    setTimeout(function() {
    ...
    function useVoucher(e) {
        $.ajax({
            type: 'POST',
            url: '{{ route("store.checkout.use-voucher", alias()) }}',
            data: {
                _token: "{{ csrf_token() }}",
                voucher_code: $('input[name=voucher_code]').val(),
                price: $('input[name=price]').val(),
...
@include('shared.pixel_scripts', ['reseller' => @$reseller, 'product' => $product])
@endsection